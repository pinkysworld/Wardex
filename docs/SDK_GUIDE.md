...
The `v1.0.23` clients expose the same resilience/proof contracts used by the admin console and release gate, plus the workflow-continuity tranche, the CI-repair release helpers, and the refreshed documentation-release surfaces:
...
client.release_upgrade_rehearsal(target_version="1.0.23")
...
await client.releaseUpgradeRehearsal({ targetVersion: "1.0.23" });
