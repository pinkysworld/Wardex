...
`v1.0.5` is the current stable patch release of Wardex. It carries all capabilities of `v1.0.0`, the `v1.0.1` CI hardening fixes, the `v1.0.2` macOS release-trust hardening, the Live Monitor process-analysis regression coverage, and the refreshed Developer ID `.p12` signing-secret repair path for notarized macOS release archives.
