...
- Federated SSO launch and callback validation now includes provider launch checks for metadata, callback route alignment, client credentials, group mappings, and test-login paths. The next tranche is routed browser regression depth around ready-provider launch paths, callback/session recovery, and login-shell coherence as providers evolve.
