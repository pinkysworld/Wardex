...
The current release adds failed-auth request bucketing across IP-wide and hashed presented-token dimensions, lock-label budget drop metrics, read-heavy feature-flag concurrency, Live Monitor selected-alert restore, a Failed Auth audit pivot, Google Workspace collector form reuse, Detection Explain contract parity, and a documented `make smoke` release gate.
