...
fn failover_drill_endpoint_records_latest_result() {
    ...
    let drill: serde_json::Value = ureq::post(&format!("{base_url}/api/control/failover-drill"))
        .set("Authorization", &auth_header)
        .call()
...
fn product_contract_endpoint_inventory_is_complete() {
    ...
    assert!(product_contract_missing_from_source(include_str!("../docs/openapi.yaml")).is_empty());
    assert!(
...
fn auto_progress_records_verified_update_signature_metadata() {
    ...
    let (canary_id, ring_id, signer_pubkey, payload_hash, counter) = {
        ...
        let ring_id = enroll_test_agent(&mut state.agent_registry, "ring-host", "linux", "1.0.0");
        let release = state
            .update_manager
...
fn sso_login_callback_creates_cookie_backed_session() {
    fn spawn_mock_oidc_provider() -> (
        ...
        let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind mock oidc provider");
    ...
    let (issuer_url, token_bodies, expected_nonce, provider_handle) = spawn_mock_oidc_provider();
    let (port, token) = spawn_test_server();
    let base_url = format!("http://127.0.0.1:{port}");
    let auth_header = format!("Bearer {token}");
    ...
    let location = login_response
        ...
        .to_string();
    assert!(location.starts_with(&format!("{issuer_url}/authorize?")));
...
fn idp_provider_endpoint_saves_and_rejects_invalid_configs() {
    ...
    assert_eq!(
        sso_config["providers"].as_array().map(std::vec::Vec::len),
        Some(1)
    );
    assert_eq!(
...
fn assistant_status_and_query_include_case_context_and_citations() {
    ...
    assert_eq!(
        body["citations"].as_array().map_or(0, std::vec::Vec::len),
        2
    );
...
fn build_workbench_overview_aggregates_queue_cases_incidents_and_response() {
    ...
    assert!(
        overview.urgent_items.iter().any(|item| item.kind == "queue"
            || item.kind == "response"
