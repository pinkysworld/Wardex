...
impl EfficacyTracker {
    ...
    pub fn per_rule_efficacy(&self) -> Vec<RuleEfficacy> {
        ...
        by_rule
            ...
            .map(|(rule_id, records)| {
                ...
                let tp = records
                    .iter()
                    .filter(|r| r.outcome == AlertOutcome::TruePositive)
                    .count();
                let fp = records
                    .iter()
                    .filter(|r| r.outcome == AlertOutcome::FalsePositive)
                    .count();
                let benign = records
                    .iter()
                    .filter(|r| r.outcome == AlertOutcome::Benign)
                    .count();
                let inconclusive = records
                    .iter()
                    .filter(|r| r.outcome == AlertOutcome::Inconclusive)
                    .count();
                let pending = records
                    .iter()
                ...
                let tp_rate = if resolved > 0 {
                    tp as f32 / resolved as f32
                } else {
                    0.0
                };
                let fp_rate = if resolved > 0 {
                    fp as f32 / resolved as f32
                } else {
                    0.0
                };
                ...
                let triage_times: Vec<f32> = records
                    .iter()
                    .filter(|r| r.outcome != AlertOutcome::Pending)
                    .map(|r| r.triage_duration_ms as f32 / 1000.0)
                    .collect();
                let mean_triage = if triage_times.is_empty() {
                    0.0
                } else {
                    triage_times.iter().sum::<f32>() / triage_times.len() as f32
                };
                ...
                let trend = if records.len() < 10 {
                    ...
                    let first_half_fp = records[..mid]
                        .iter()
                        .filter(|r| r.outcome == AlertOutcome::FalsePositive)
                        .count() as f32
                        / mid as f32;
                    let second_half_fp = records[mid..]
                        .iter()
                        .filter(|r| r.outcome == AlertOutcome::FalsePositive)
                        .count() as f32
                        / (records.len() - mid) as f32;
    ...
    pub fn summary(&self) -> EfficacySummary {
        ...
        let triaged: Vec<&TriageRecord> = self
            .records
            .iter()
            .filter(|r| r.outcome != AlertOutcome::Pending)
            .collect();
        ...
        let overall_tp = if resolved > 0 {
            tp as f32 / resolved as f32
        } else {
            0.0
        };
        let overall_fp = if resolved > 0 {
            fp as f32 / resolved as f32
        } else {
            0.0
        };
        ...
        let triage_times: Vec<f32> = triaged
            .iter()
            .map(|r| r.triage_duration_ms as f32 / 1000.0)
            .collect();
        let mean_triage = if triage_times.is_empty() {
        ...
        let sev_map: HashMap<String, SeverityEfficacy> = by_severity
            .into_iter()
            .map(|(sev, recs)| {
                ...
                let s_resolved = s_tp
                    ...
                    + recs
                        ...
                        .filter(|r| r.outcome == AlertOutcome::Benign)
                        .count();
                let s_triage: Vec<f32> = recs
                    .iter()
                    .map(|r| r.triage_duration_ms as f32 / 1000.0)
                    .collect();
                let s_mean = if s_triage.is_empty() {
                    0.0
                } else {
                    s_triage.iter().sum::<f32>() / s_triage.len() as f32
                };
                (
                    sev,
                    SeverityEfficacy {
                        total: s_total,
                        tp_rate: if s_resolved > 0 {
                            s_tp as f32 / s_resolved as f32
                        } else {
                            0.0
                        },
                        fp_rate: if s_resolved > 0 {
                            s_fp as f32 / s_resolved as f32
                        } else {
                            0.0
                        },
        ...
        let mut worst: Vec<RuleEfficacy> = rules
            .iter()
        ...
        let mut best: Vec<RuleEfficacy> = rules
            .iter()
            .filter(|r| r.total_alerts >= 5)
            .cloned()
            .collect();
