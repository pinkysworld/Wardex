...
### Added
- **Real kernel-driven telemetry**: `src/kernel_linux/` adds a Linux backend built on the `CN_PROC` netlink connector for process lifecycle (fork/exec/exit/uid-change, requires `CAP_NET_ADMIN`) and `fanotify(7)` (falling back to `inotify(7)`) for file activity, replacing the previous "eBPF" label on top of `/proc` polling — eBPF itself is not implemented (see the `ebpf` feature and `KernelBackendKind::Ebpf` for why). `src/kernel_windows/` adds a real-time ETW consumer for the kernel process/file/network manifest providers plus best-effort DNS-Client, PowerShell ScriptBlock, and AMSI telemetry (requires elevation), replacing a `has_etw: bool` flag next to the WMI-only collector. `src/kernel_macos/` adds a macOS Endpoint Security (ESF) client behind the opt-in, off-by-default `macos-es` feature, requiring the `com.apple.developer.endpoint-security.client` entitlement, TCC Full Disk Access, and root — not exercised end-to-end in this environment. Event-mapping logic for both Windows and macOS is pure, platform-independent, and unit-tested on every host.
- **Federated learning with differential privacy** (`src/federated.rs`, `src/server_federated.rs`): a coordinator/participant protocol for cross-agent federated averaging over the existing authenticated agent↔server channel. An operator opens a federation round against the current global model; agents train locally, clip their update to an L2-norm bound `C`, and add Gaussian noise calibrated to the round's `(epsilon, delta)` budget before submitting it, building on the in-process DP primitives in `src/privacy.rs`. New `[federation]` config section (disabled by default); see `docs/FEDERATED_LEARNING.md`.
- **Threat-intelligence enrichment** (`src/threat_intel_enrich.rs`): VirusTotal (v3) and AbuseIPDB (v2) clients for file-hash, IP, domain, and URL lookups, with per-provider rate limiting, response caching, timeouts, and graceful degradation when a provider is unreachable, misconfigured, or rate-limited.
- **Bidirectional ticketing integrations** (`src/ticketing.rs`): real Jira Cloud/Server and ServiceNow Table API clients with idempotent create-or-update semantics, so retries never create duplicate remote tickets; `enterprise_store::TicketSyncRecord` remains the local system of record and now syncs remote status back in.
- **OTLP export, SMTP STARTTLS, and Okta polling** integrations wired through `src/server_integrations_ext.rs`, following the same storage/secrets pattern as the existing cloud/identity collectors.
...
- **Live Docker/Podman and Kubernetes container event sources**: `src/container_runtime.rs` adds a minimal HTTP/1.1 Docker Engine API client over a Unix socket (`/_ping`, `/version`, `/containers/json`, inspect, and the streaming `/events` endpoint with reconnect/backoff and `since` resume), feeding container start/die/exec/pull/network events and inspected privileged/capability/mount state into the existing `ContainerDetector` rules. Also adds a best-effort in-cluster Kubernetes Pod-watch client and event mapping (privileged/hostPID/hostNetwork/hostPath), documented as TLS-trust-limited pending a custom CA anchor. Reachability surfaces in `wardex doctor` and `GET /api/platform`. New `[container]` config section.
- **Previously documented-but-missing config keys implemented**: `[collection]` (`collection_interval_secs`, `max_events_per_batch`), `[detection]` (`profile`, `anomaly_threshold`, `slow_attack_window_secs`, `ransomware_canary_dirs`), `[collectors]` (`ebpf_enabled`, `ebpf_programs`, `etw_enabled`, `wmi_enabled`, `amsi_enabled`, and the registry/process/network scan intervals), `[relay]` (`sync_interval_secs`, ...), and `[attestation]` (`trust_store_path`, ...) now exist in `Config` with serde defaults, are wired to real behaviour (agent collection-loop cadence, the low-and-slow detector's window, Windows backend capability gating, and a new `wardex attest-verify` CLI command), and are covered by a test that extracts the TOML snippets from `docs/CONFIGURATION.md` and the Linux/Windows runbooks and deserializes them against the real schema. `collector.gcp.service_account_key_path` is accepted as a serde alias for the existing `key_file_path`.
### Changed
- **Real Tantivy-backed search**: `search.rs` previously claimed a Tantivy full-text index but only ever scanned an in-memory `Vec` rebuilt from scratch on every `/api/search`/`/api/hunt` request. It is now a genuine, mmap'd, on-disk Tantivy index with a DATE fast/sort field, STRING keyword fields (substring/wildcard via compiled regex queries), and TEXT fields for message/cmdline; the existing hunt-query DSL compiles directly to native `BooleanQuery`/`RegexQuery`/`RangeQuery`. Events are indexed incrementally on ingest (single mutex-guarded writer, shared reader, no per-request rebuild), retention deletion runs as a range delete keyed off the `timestamp` fast field, and a schema-version mismatch or open failure triggers an automatic rebuild from the primary event store. Added `/api/search/status` and `/api/search/rebuild`, `search.*` config with serde defaults (index path, writer memory budget, commit interval, retention days), and a `wardex doctor` check reporting index doc count/size/last commit.
- **Real WebAssembly extension runtime**: `src/wasm_runtime.rs` replaces the aspirational docs/site description of "Wasm extensions" with an actual `wasmi`-based (pure-Rust interpreter, no JIT) sandbox: fuel-metered CPU budget, memory-page limits via `ResourceLimiter`, load-time module size/import/export validation, and a versioned `wardex_v1` host ABI (`log`, `emit_alert`, `kv_get`/`kv_set`, `now_unix_ms`) documented in `docs/WASM_ABI.md`. Extensions load from a configured directory or via `POST /api/wasm-extensions/upload` (SHA-256 hash pinning, optional Ed25519 signature checking reusing the `auto_update.rs` signing pattern), with per-extension metrics (invocations, fuel used, traps, errors, alerts emitted) surfaced via `GET /api/wasm-extensions` and `wardex doctor`. The prior stack-based bytecode VM (`src/wasm_engine.rs`, `PolicyVm`) is unchanged and kept for its one existing caller (`POST /api/policy-vm/execute`), now documented as legacy. See `docs/WASM_TUTORIAL.md` and `examples/wasm_extensions/` for a WAT and a Rust guest example.
- **Dependency and security updates**: `rustls` bumped to `0.23.45` for `RUSTSEC-2026-0285`, plus major-version upgrades to `rand` (0.10), `ed25519-dalek` (3), `aes-gcm` (0.11), and `toml` (1) across the workspace.
- **Admin console topbar and navigation redesign**: pseudo-icon glyphs replaced with a shared inline SVG icon set (`components/icons.jsx`), severity-aware KPI tiles, and quieter topbar chrome; the topbar's "More" overflow menu now closes on route change, `Escape`, and outside click, and Help For View / Share Link render inline again on wide viewports.
